...
class cartController {
    ...
    async getCartById(req, res) {
        ...
        try {
            ...
            if (json) res.status(200).json(result)
            else res.render('cartById', {
                result,
                title: "Search Cart",
                styleRoute: `<link href="/styles/cartbyid.css" rel="stylesheet">`
            })
        } catch (error) {
    ...
    async createCart(req, res) {
        ...
        try {
            ...
            //Activar para enviar correo
            /* await transport.sendMail({
                from: 'Elisabeth <eesaez.munoz@gmail.com>',
                to: req.user.user,
                subject: 'Carrito nuevo creado',
                html: `<div><h1> Hey! Has creado un carrito exitosamente! </h1></div>`,
                attachments: []
            }) */
